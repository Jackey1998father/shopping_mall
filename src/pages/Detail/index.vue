    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imgUrl="skuInfo.skuDefaultImg" :skuImageList="skuImageList"></Zoom >
          <!-- 小图列表 -->
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spu in spuSaleAttrList" :key="spu.id" >
                ...
                <dd @click="fun(valueName,spu.spuSaleAttrValueList)" :condition="spu.id" changepirce="0" :class="{active:valueName.isChecked == 1}" v-for="(valueName) in spu.spuSaleAttrValueList" :key="valueName.id">{{valueName.saleAttrValueName}}</dd>   
              </dl>                                                                                                                                                
              
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model.number="skuNum" @change="skuChange"/>
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum >1? skuNum--:skuNum=1 ">-</a>
              </div>
              <div class="add">
                <a  @click="reqAddOrUpdateShopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async reqAddOrUpdateShopCart(){ 
      try {
        //成功 
        // 发送添加的商品到购物车的数据到服务器：
        await this.$store.dispatch("reqAddOrUpdateShopCart",{skuId:this.$route.params.skuid,skuNum:this.skuNum})
         // 点击实现路由跳转：
        this.$router.push({
          name:'addcartsuccess',
          query:{
    ...
    getData(value) {
      this.$store.dispatch("reqGoodsInfo", value);
      
    },
    fun(valueName,spuSaleAttrValueList){   
      console.log(valueName,spuSaleAttrValueList);
      for(let i= 0 ;i < spuSaleAttrValueList.length ; i++){
        spuSaleAttrValueList[i].isChecked = 0
      }
      valueName.isChecked = 1
      
      console.log(valueName.saleAttrValueName);
  ...
  computed:{
    ...mapGetters(["categoryView","price","skuInfo","valuesSkuJson","spuSaleAttrList"]),
    skuImageList(){
